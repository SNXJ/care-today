...
const needsDateTime = computed(() => ['event', 'temperature', 'symptom'].includes(selectedType.value));
const scoreFields = [
  ...
  try {
    switch (selectedType.value) {
      ...
      case 'notice':
        ...
        await api.createNotice(id, { content: form.content.trim(), detail: form.note.trim() || undefined, important: form.important, startsOn: form.date }); break;
      case 'question':
...
<template><view class="page" style="padding-top:32rpx">
  ...
  <view v-else class="card"><view class="card-title"><text>{{ current?.label }}</text><text class="tag" @click="selectedType = ''">换一种</text></view>
    ...
    <view class="date-grid"><view class="field"><text class="label">日期</text><picker mode="date" :value="form.date" @change="form.date = $event.detail.value"><view class="picker">{{ form.date }}</view></picker></view><view v-if="needsDateTime" class="field"><text class="label">时间</text><picker mode="time" :value="form.time" @change="form.time = $event.detail.value"><view class="picker">{{ form.time }}</view></picker></view></view>
